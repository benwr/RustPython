...
def S_IFMT(mode):"Return the portion of the file's mode that describes the\n    file type.\n    ";return mode&61440
S_IFDIR=16384
S_IFCHR=8192
S_IFBLK=24576
S_IFREG=32768
...
def S_ISDOOR(mode):'Return True if mode is from a door.';return _A
def S_ISPORT(mode):'Return True if mode is from an event port.';return _A
def S_ISWHT(mode):'Return True if mode is from a whiteout.';return _A
S_ISUID=2048
S_ISGID=1024
S_ENFMT=S_ISGID
S_ISVTX=512
...
def filemode(mode):
	"Convert a file's mode to a string of the form '-rwxrwxrwx'.";perm=[]
	for table in _filemode_table:
		for(bit,char)in table:
			if mode&bit==bit:perm.append(char);break
		else:perm.append('-')
	return''.join(perm)
